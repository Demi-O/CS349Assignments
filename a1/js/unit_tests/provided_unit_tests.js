...
describe('First unit test', function() {
    ...
    it('Listener unit test for GraphModel', function() {
        ...
        graphModel.selectGraph("Data Entries");

        expect(firstListener.called, 'GraphModel listener should be called').to.be.ok;
        expect(firstListener.calledWith('GRAPH_SELECTED_EVENT', sinon.match.any, 'Data Entries'), 'GraphModel argument verification').to.be.true;
    ...
    it('Test One: Listener unit test for ActivityStoreModel', function() {
        var activityModel = new ActivityStoreModel();
        var firstListener = sinon.spy();

        activityModel.addListener(firstListener);
        var activity = "Attending Lectures";
        var dictionary = {};
        dictionary["Energy Level"] = 4;
        dictionary["Stress Level"] = 2;
        dictionary["Happiness Level"] = 4;
        var durationValue = 50;
        var data = new ActivityData(activity, dictionary, durationValue);

        activityModel.addActivityDataPoint(data);

        expect(firstListener.called, 'ActivityModel listener should be called').to.be.ok;

        var secondListener = sinon.spy();
        activityModel.addListener(secondListener);
        activity = "Coding";
        var dictionaryTwo = {};
        dictionaryTwo["Energy Level"] = 3;
        dictionaryTwo["Stress Level"] = 5;
        dictionaryTwo["Happiness Level"] = 3;
        durationValue = 60;
        var dataTwo = new ActivityData(activity, dictionary, durationValue);

        activityModel.addActivityDataPoint(dataTwo);
        expect(firstListener.callCount, 'ActivityModel first listener should have been called twice').to.equal(2);
        expect(secondListener.called, "ActivityModel second listener should have been called").to.be.ok;
    });

    it('Test Two: Add event unit test for ActivityStoreModel', function() {
        var activityModel = new ActivityStoreModel();
        var firstListener = sinon.spy();
        ...
        activityModel.addActivityDataPoint(data);
        expect(firstListener.calledWith('ACTIVITY_DATA_ADDED_EVENT', sinon.match.any, sinon.match.any), 'ActivityModel argument verification').to.be.true;

    });

    it('Test Three: Remove event unit test for ActivityStoreModel', function() {
        var activityModel = new ActivityStoreModel();
        var firstListener = sinon.spy();
        ...
        expect(firstListener.calledWith('ACTIVITY_DATA_REMOVED_EVENT', sinon.match.any, sinon.match.any), 'ActivityModel argument verification').to.be.true;

    });

    it('Test Four: Correct data unit test for ActivityStoreModel(add and delete event)', function() {
        var activityModel = new ActivityStoreModel();
        var firstListener = sinon.spy();
        ...
        activityModel.addActivityDataPoint(data);
        expect(firstListener.calledWith('ACTIVITY_DATA_ADDED_EVENT', sinon.match.any, data), 'ActivityModel argument verification').to.be.true;

        activityModel.removeActivityDataPoint(data);
        expect(firstListener.calledWith('ACTIVITY_DATA_REMOVED_EVENT', sinon.match.any, data), 'ActivityModel argument verification').to.be.true;

    });

    it('Test Five: Selected graph unit test for GraphModel', function() {
        var graphModel = new GraphModel();
        var firstListener = sinon.spy();

        graphModel.addListener(firstListener);
        graphModel.selectGraph("Summary");
        expect(graphModel.selectedGraph=="Summary", 'Selected graph == Summary').to.be.ok;

        graphModel.selectGraph("Data Entries");
        expect(graphModel.selectedGraph=="Data Entries", 'Selected graph == Data Entries').to.be.ok;

        graphModel.selectGraph("Chart");
        expect(graphModel.selectedGraph=="Chart", 'Selected graph == Chart').to.be.ok;
